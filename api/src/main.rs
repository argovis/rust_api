...
async fn search_data_schema(query_params: web::Query<serde_json::Value>) -> impl Responder {
    ...
    // Dataset-specific request-size policy. Drives tile sizing in Step 6
    // and radius capping in Step 7.
    ...
    //
    // Step 5: route the filter through the tile-aware composer. The tile
    // is a *passthrough* — no spatial bbox, no level bracket — so the
    // composer returns the user filter unchanged and behavior is identical
    // to the pre-pagination handler. Step 6 will replace this with a loop
    // over real tiles generated by `tile_generator::generate_tiles`.
    let tile = tile_generator::TileSpec {
        tile_bbox: None,
        level_index: None,
    };
